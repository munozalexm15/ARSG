...
func enter(_msg := {}):
	...
	if (not _msg.has("replace_weapon") and not _msg.has("isSwappingValue")):
		...
		if (state_machine.old_state.name == "Reload" or state_machine.old_state.name == "SwappingWeapon" or state_machine.old_state.name == "Grenade") and Input.is_action_pressed("Sprint"):
